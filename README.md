...
> Author: bluechips | Version: 1.0.1
...
- **测试执行不再阻塞 UI**：测试流程移至 `QThread` 异步执行，界面保持流畅响应
- **安全检测器全面集成**：IDOR、认证绕过、上传漏洞检测器现已集成到测试流程，不再仅初始化不调用
- **JWT 检测结果保存**：JWT 分析结果现在正确存入 `TestResult`，可在详情中查看
- **TestResult 数据模型完善**：新增 `jwt_info`、`idor_info`、`auth_bypass_info`、`upload_info` 字段

#### 🟡 重要改进

- **停止测试功能实现**：点击"停止测试"按钮现在可以真正中断正在运行的测试
- **安全检测器 UI 开关**：设置面板新增 5 个检测器开关（敏感信息、JWT、IDOR、认证绕过、上传漏洞），无需修改代码即可启用
- **Fuzzer 工具栏入口**：工具栏新增 Fuzzer 按钮，支持选择 Payload 类别进行参数变异测试
- **多格式导出**：导出结果现在支持 Excel、CSV、JSON、HTML 四种格式选择
- **状态码过滤**：过滤功能已实现，可按状态码筛选测试结果
- **设置实时生效**：修改检测器设置后自动重建 TestExecutor，无需重启程序
- **HTTP 客户端双重重试修复**：移除 urllib3 层的状态码重试，避免实际重试次数远超预期

#### 🎨 UI 改进

- **全新深海暗夜配色**：替换紫色主题为深蓝-青色配色方案，更加专业美观
- **丝滑动画效果**：窗口入场淡入+上滑动画、按钮悬停缩放动画、分组框阴影动画
- **免责声明标签页**：设置面板新增免责声明页面，包含完整法律声明和使用限制
- **拦截方法横向排列**：安全设置中的拦截方法改为横向复选框，不再需要滚动
