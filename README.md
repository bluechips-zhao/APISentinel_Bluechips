...
> Author: bluechips | Version: 1.1.1
...
- **Fuzzer 功能修复**：Fuzzer 按钮点击后实际执行的是普通测试而非参数变异测试，现已使用专用 `_FuzzWorker` 线程正确调用 `fuzzer.test_endpoint()`
- **代理设置修复**：代理仅设置到 MainWindow 的 HttpClient，未传播到 TestExecutor 内部的 HttpClient，导致测试请求不走代理
- **安全模式修复**：SafeMode 的 `filter_endpoints()` 从未在测试流程中调用，安全模式设置（拦截方法、危险关键词、URL 黑名单）完全无效
- **停止测试修复**：停止按钮不支持中断 Fuzzing 过程，现已同时支持停止测试和 Fuzzing

#### 🟡 重要改进

- **Fuzzer 支持代理**：Fuzzing 测试现在也会使用代理设置
- **安全模式提示**：接口被安全模式过滤时在状态栏显示过滤数量，全部过滤时弹窗提醒用户
- **Fuzzer 结果实时显示**：Fuzzing 过程中结果实时显示到结果表格，而非等待全部完成
